...
Both programs are deployed to Solana devnet (IDs in [Anchor.toml](Anchor.toml)) with full state seeded — Config, the 25-constituent registry, IndexState, Market, and an InsuranceFund seeded via `deposit_insurance`. A standalone **publisher crank** runs as an always-on Railway worker (`publisher-crank`): it polls every 60s and, whenever the previous UTC day hasn't been submitted + aggregated yet, scrapes real eBay-derived PSA 10 prices (via the Oxylabs scraper service), submits them, and calls `aggregate_day` — so the index self-updates within ~1 min of each midnight-UTC rollover and a failed cycle retries on the next tick. The index moves **once per UTC day** by design: that's the resolution of the underlying sold-price data, so the index is a slow, hard-to-manipulate settlement anchor while the vAMM mark moves continuously on trader flow and funding reconciles the spread (see [docs/perp-engine.md](docs/perp-engine.md) §4).

Trading is **already enabled on-chain** — the market runs full Phase-1 risk params (3× max leverage, 50k/trader cap, 500k OI/side) with `trading_paused = false` (the `phase` field still reads 0, but `open_position` gates on the pause flag, not the phase). Opening and closing positions is verified working end-to-end through the deployed dashboard with a wallet on devnet. A second Railway worker, **`keeper`** (`services/keeper/`), runs the market mechanics: every 60s it calls `settle_funding` (gated so funding accrues at most hourly) and scans every open position, liquidating any whose equity has fallen below maintenance margin. Between the index crank and the keeper the market self-maintains — index, funding, and liquidations all advance with no manual cranking. The one piece left for broad public testing is a test-USDC faucet (the mint authority is currently the deploy admin).

**Operational hardening (devnet).** The admin authority on *both* programs has been migrated to a **2-of-3 Squads V4 multisig** via the v0.8 two-step transfer flow — `Config.admin` and `Market.admin` now point at the Squad vault PDA, so no single key can pause the market, rebalance the registry, or pull treasury. The Squad, vault, and migration scripts live in `services/dashboard/scripts/squads/`. A third Railway worker, **`monitor`** (`services/monitor/`), polls protocol *outcomes* every 2 min — oracle freshness, funding advancement, bad debt, insurance floor, indexer `/health`, and keeper SOL balance — and fires Telegram alerts on OK↔BAD transitions, so it catches "service running but not doing its job," not just process death. The dashboard routes all RPC through a server-side proxy (`/api/rpc`) so the paid Helius endpoint key never reaches the browser, and serves public protocol docs at **`/docs`**. The mainnet bring-up sequence, signer composition, and incident runbook are written up in [docs/mainnet-runbook.md](docs/mainnet-runbook.md) (internal) with a secrets/key inventory in [docs/secrets.md](docs/secrets.md).
