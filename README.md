...
- `auto_deleverage` ranking is "probabilistic" — caller passes ≥ 1 witness, candidate must out-PnL each, but the proof doesn't enforce "globally highest". v0.5 could require N witnesses tied to total open-position count.
- `auto_deleverage` pays out margin + full pnl at index price — no haircut. To actually *restore* the insurance fund (the spec's "socialize the loss" mechanic), the trader's payout should be haircutted. v0.5 follow-up.
