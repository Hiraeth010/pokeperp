...
Working v0.7 end-to-end on localnet. Both on-chain programs are implemented and verified; off-chain services run; the dashboard renders live on-chain state and the test wallet can open / modify / close positions through a real Anchor flow. **94 tests passing across the workspace** (18 oracle integration + 19 perp-engine integration + 5 funding-math unit + 52 publisher unit; 1 perp-engine test self-skips when the liquidation-test EMA hasn't converged).
...
**v0.7 known gaps** (documented in code)

- Treasury withdrawal is admin-only (single signer = Market.admin). DAO / multisig governance hook is a v0.8 follow-up.
- Challenge resolution is admin-attested (admin picks the slash tier). Committee multisig + on-chain deviation computation against `IndexState.aggregated_prices` is v0.8. Spec §7's "25% to remaining publishers" distribution is currently rolled into the 50% treasury share (no N-publisher fan-out yet).
