...
Working v0.8 end-to-end on localnet. Both on-chain programs are implemented and verified; off-chain services run; the dashboard renders live on-chain state and the test wallet can open / modify / close positions through a real Anchor flow. **100 tests passing across the workspace** (19 oracle integration + 24 perp-engine integration + 5 funding-math unit + 52 publisher unit; 1 perp-engine test self-skips when the liquidation-test EMA hasn't converged).

**Implemented**

- **Oracle program** (16/16 instructions): config + publisher onboarding, 25-slot constituent registry with chunked rebalance updates, daily push submissions with on-chain median aggregation + provisional/final lifecycle, challenge open/resolve with **real bond escrow + slashing** (per-challenge bond vault PDA escrows the challenger's 1k USDC stake; resolve_challenge takes a `slash_bps` ∈ {1000, 5000, 10000} on success, slashes the targeted publisher's bond and splits 50/50 between challenger USDC ATA + protocol treasury vault, refunds the challenger's bond; failure splits the challenger's bond 50/50 between publisher's bond vault refill and the treasury; auto-suspend at ≥50% slash, auto-remove at 100%), `set_protocol_treasury` admin setter that wires the perp-engine treasury vault into oracle Config (must be called post-perp-init; init-localnet does this in step 7c), emergency pause, **v0.8 two-step admin transfer** (`propose_admin_transfer` + `accept_admin_transfer`) for migrating Config.admin to a Squads multisig vault PDA without leaving a single-key window.
...
**v0.8 known gaps** (documented in code)
...
- Challenge resolution is still admin-attested (admin picks the slash tier). Once the multisig is wired, the slash decision lives behind the Squad's M-of-N approval, but spec §7's committee multisig + on-chain deviation computation against `IndexState.aggregated_prices` is still not implemented — the slash tier is governance-judged, not arithmetic.
...
- `tests/oracle.ts` (19 tests): Config init, publisher register + bond escrow + activate-shadow-period gating, submit_price_update + duplicate rejection, registry init + update + finalize, aggregate_day all-stale path, **challenge open with bond escrow, resolve-success with 10% slash + dual-distribution verification, resolve-failure with 50/50 publisher-refill / treasury split**, emergency pause, **v0.8 two-step admin transfer** (propose → accept → restore, with old-admin loses-authority assertion).
