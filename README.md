...
Working v0.6 end-to-end on localnet. Both on-chain programs are implemented and verified; off-chain services run; the dashboard renders live on-chain state and the test wallet can open / modify / close positions through a real Anchor flow. **90+ tests passing across the workspace** (18 oracle integration + 15 perp-engine integration + 5 funding-math unit + 52 publisher unit).

**Implemented**

- **Oracle program** (14/14 instructions): config + publisher onboarding, 25-slot constituent registry with chunked rebalance updates, daily push submissions with on-chain median aggregation + provisional/final lifecycle, challenge open/resolve with **real bond escrow + slashing** (per-challenge bond vault PDA escrows the challenger's 1k USDC stake; resolve_challenge takes a `slash_bps` ∈ {1000, 5000, 10000} on success, slashes the targeted publisher's bond and splits 50/50 between challenger USDC ATA + protocol treasury vault, refunds the challenger's bond; failure splits the challenger's bond 50/50 between publisher's bond vault refill and the treasury; auto-suspend at ≥50% slash, auto-remove at 100%), `set_protocol_treasury` admin setter that wires the perp-engine treasury vault into oracle Config (must be called post-perp-init; init-localnet does this in step 7c), emergency pause.
- **Perp engine program** (14/14 instructions): isolated-margin positions, oracle-anchored vAMM mark price (`index × (1 + slippage_factor × imbalance)`), per-trade mark-TWAP EMA on open / close / modify, insurance-mediated PnL settlement on close with **v0.6 self-ADL fallback** (insurance shortfall haircuts the closing trader + emits `InsuranceShortfall` event instead of reverting), per-position funding settlement on close + liquidate (now with actual cash flow through the insurance vault, not just the equity-check gate) + modify (settles against pre-modify size, re-snapshots, then resizes), open + close taker fees split 90% to protocol Treasury + 10% to InsuranceFund per spec §9, admin-gated `withdraw_treasury` for protocol revenue pull (single-signer for v0.4; DAO/multisig is v0.5), liquidation with penalty split (1/3 liquidator, 2/3 insurance), `auto_deleverage` with on-chain ranking via witness positions in `remaining_accounts` (each witness must be same-side + same-market + have PnL ≤ candidate's at the same index_price).
- **Publisher binary** (`services/publisher/`): standalone Rust service. Reads the on-chain registry, computes prices via either a deterministic drift path (for localnet/staging) or the real eBay Browse source with the methodology pipeline (PSA 10 regex + qualifier rejection, English-only CJK filter, variant matching, multi-window fallback, trimmed mean), with a Card-Codex aggregate-price fallback when listings are insufficient. Persistent `<config>.state.json` sidecar tracks per-constituent `last_fresh_day` for the stale-decay fallback. Real merkle root over selected leaves. `verify --day N [--tolerance-bps M]` re-runs a day's computation locally and compares to the on-chain submission for post-hoc audit; reports per-slot deviation and source-root match, non-zero exit on disagreement.
...
**v0.6 known gaps** (documented in code)

- `auto_deleverage` ranking is "probabilistic" — caller passes ≥ 1 witness, candidate must out-PnL each, but the proof doesn't enforce "globally highest". v0.7 could require N witnesses tied to total open-position count.
- `auto_deleverage` pays out margin + full pnl at index price — no haircut. To actually *restore* the insurance fund (the spec's "socialize the loss" mechanic), the trader's payout should be haircutted. v0.7 follow-up.
- **v0.6 added the insurance-shortfall self-ADL fallback**: `close_position` and `liquidate` no longer revert when insurance can't fully cover a payout — they top up by what's available and haircut the closing/liquidated trader by the deficit, emitting `InsuranceShortfall { kind, owed, paid, shortfall }` so off-chain cranks can recapitalize via the explicit `auto_deleverage` ix. Liquidation margin shortfall now draws from insurance (liquidator share gets priority over insurance share, preserving the liquidator incentive) instead of silently burning. Integration test for the actual trigger is a follow-up — the math is straightforward Rust + happy path is unchanged.
- Treasury withdrawal is admin-only (single signer = Market.admin). DAO / multisig governance hook is a v0.6 follow-up.
- Challenge resolution is admin-attested (admin picks the slash tier). Committee multisig + on-chain deviation computation against `IndexState.aggregated_prices` is v0.6. Spec §7's "25% to remaining publishers" distribution is currently rolled into the 50% treasury share (no N-publisher fan-out yet).
- Liveness slashing (§7's 5% / 25% tiers for missed submissions) is a separate mechanism not yet implemented.
- Publisher: real eBay Browse sold-items access still needs Marketplace Insights partner approval. Card-Codex aggregate fallback wired in v0.4 with a 23-of-25 inception-card URL mapping (Shining Fates Shiny Vault uses SV-prefixed numbers that don't fit Card-Codex's collector-number URL convention — deferred).
