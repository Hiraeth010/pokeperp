...
Working v0.5 end-to-end on localnet. Both on-chain programs are implemented and verified; off-chain services run; the dashboard renders live on-chain state and the test wallet can open / modify / close positions through a real Anchor flow. **89+ tests passing across the workspace** (18 oracle integration + 14 perp-engine integration + 5 funding-math unit + 52 publisher unit).
...
**v0.5 known gaps** (documented in code)

- `auto_deleverage` ranking is "probabilistic" — caller passes ≥ 1 witness, candidate must out-PnL each, but the proof doesn't enforce "globally highest". v0.6 could require N witnesses tied to total open-position count.
- `auto_deleverage` pays out margin + full pnl at index price — no haircut. To actually *restore* the insurance fund (the spec's "socialize the loss" mechanic), the trader's payout should be haircutted. v0.6 follow-up.
- `InsuranceBelowFloor` revert on `close_position` and `liquidate` doesn't auto-fall-back to ADL. A caller must run `auto_deleverage` explicitly to restore insurance, then retry the failed close/liquidate.
