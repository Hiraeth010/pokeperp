...
A Solana perpetual futures DEX settling against the **PSA 10 Modern Top 50** Pokemon card index (PMT50) — the 50 most-traded modern-era PSA 10 graded cards by trailing 90-day eBay sold dollar volume.  Expanded from PMT25 to PMT50 in v0.10 (May 2026) — see [the v0.10 expansion section](#v010-index-expansion-2550) below.

![Pokeperp index page — PMT50 hero, Top Movers card grid, Market State](docs/screenshots/index.png)
...
- **Localnet seed**: `init-localnet.ts` seeds the full 50-constituent PMT50 list (PMT1-25 inception list plus the v0.10 PMT26-50 expansion, validated by trailing 90-day eBay sold $ volume). `seed-index.ts` submits matching prices so the dashboard reads $1000.00 with 0% drift across all 50 cards on a fresh bring-up.
